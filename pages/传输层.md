...
## 若第三次握手服务器未接收到客户端发送过来的 ACK 报文，同样会采取类似于客户端的超时重传机制，若重传次数超过限制后仍然没有回应，则 accep() 系统调用返回 -1，服务器端连接建立失败。**但此时客户端认为自己已经连接成功了**，因此开始向服务器端发送数据，但是服务器端的 accept() 系统调用已返回，此时没有在监听状态。因此**服务器端接收到来自客户端发送来的数据时会发送 RST 报文给 客户端，消除客户端单方面建立连接的状态**。
## **为什么要进行三次握手？两次握手可以吗？**
...
## **SYN可以理解为是服务器端传回数据的初始序号，也可以说是确认服务器端收到的消息来自于同一个客户机**
## 为什么要四次挥手？
##
